...
#SBATCH --output=hyperparam_tuning_patchstst_sep.%j.out         # Standard output log
#SBATCH --error=hyperparam_tuning_patchstst_sep.%j.err          # Standard error log
...
do
    if [ "$stride" -le "$patch_len" ]; then
        ...
        python -u run.py \
            ...
            --data_path national_illness_24.csv \
            --model_id tmmodels2 \
            --model PatchTST_sep \
            --data custom \
            --features S \
