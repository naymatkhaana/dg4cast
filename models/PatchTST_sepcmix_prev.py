...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.head_nf_sh = configs.d_model * \
                       int((self.seq_len_sh - patch_len_sh) / stride_sh + 2)

        # Prediction Head
        self.head_nf_sh_pre = configs.d_model * \
                       int((self.seq_len_sh_pre - patch_len_sh_pre) / stride_sh_pre + 2)
    ...
    def forecast2(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # z: [bs x nvars x patch_num x d_model]
        enc_out = torch.reshape( enc_out, (-1, n_vars, int(enc_out.shape[-2]), enc_out.shape[-1]))
        ...
        enc_out_sh, attns_sh = self.encoder_sh(enc_out_sh)
        # z: [bs x nvars x patch_num x d_model]
        enc_out_sh = torch.reshape( enc_out_sh, (-1, n_vars_sh, int(enc_out_sh.shape[-2]), enc_out_sh.shape[-1]))
        # z: [bs x nvars x d_model x patch_num]
        enc_out_sh = enc_out_sh.permute(0, 1, 3, 2)
        #print("enc_out_sh.shape::::::::::::::",enc_out_sh.shape)
        ...
        enc_out_sh_pre, attns_sh_pre = self.encoder_sh_pre(enc_out_sh_pre)
        # z: [bs x nvars x patch_num x d_model]
        enc_out_sh_pre = torch.reshape( enc_out_sh_pre, (-1, n_vars_sh_pre, int(enc_out_sh_pre.shape[-2]), enc_out_sh_pre.shape[-1]))
        # z: [bs x nvars x d_model x patch_num]
        enc_out_sh_pre = enc_out_sh_pre.permute(0, 1, 3, 2)
        #print("enc_out_sh.shape::::::::::::::",enc_out_sh.shape)
