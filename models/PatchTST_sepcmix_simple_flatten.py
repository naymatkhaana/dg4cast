...
class FlattenHead(nn.Module):
    def __init__(self, n_vars, nf, target_window, head_dropout=0):
        ...
        #print("nf::::::::::",nf)
        self.linear =  nn.Linear(nf*n_vars, target_window) # nn.Linear(nf, target_window)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, x_corr):
        ...
        enc_out, attns = self.encoder(enc_out,corr_bias=corr_bias)
        ...
        enc_out = torch.reshape(
            enc_out, (-1, n_vars_flatten, int(enc_out.shape[-2]), enc_out.shape[-1]))
        ...
        n_patches = enc_out_sh.shape[1]         

        
        enc_out_sh = enc_out_sh.view(enc_out_sh.shape[0]//n_vars,n_vars,enc_out_sh.shape[1],enc_out_sh.shape[2])
        #print("************************* enc_out_sh.shape after view: ", enc_out_sh.shape)

        # Flatten vars and patches -> (batch_size, n_vars * n_patches, dim)
        enc_out_sh = enc_out_sh.reshape(enc_out_sh.shape[0], enc_out_sh.shape[1] * enc_out_sh.shape[2], enc_out_sh.shape[3])
        #print("************************* enc_out_sh.shape after flattening: ", enc_out_sh.shape)
        ...
        # Encoder
        # z: [bs * nvars x patch_num x d_model]
        enc_out_sh, attns_sh = self.encoder_sh(enc_out_sh,corr_bias=corr_bias)
        # z: [bs x nvars x patch_num x d_model]
        enc_out_sh = torch.reshape(
            enc_out_sh, (-1, n_vars_flatten, int(enc_out_sh.shape[-2]), enc_out_sh.shape[-1]))
        # z: [bs x nvars x d_model x patch_num]

        enc_out_sh = enc_out_sh.permute(0, 1, 3, 2)
        #print("enc_out_sh.shape::::::::::::::",enc_out_sh.shape)
        #print("************************* enc_out_sh.shape after permute: ", enc_out_sh.shape)
        ...
        #print("enc_out_sh.shape after patch embedding::::::::::::::",enc_out_sh.shape)
        n_patches = enc_out_sh_pre.shape[1]         
        
        enc_out_sh_pre = enc_out_sh_pre.view(enc_out_sh_pre.shape[0]//n_vars,n_vars,enc_out_sh_pre.shape[1],enc_out_sh_pre.shape[2])
        #print("************************* enc_out_sh_pre.shape after view: ", enc_out_sh_pre.shape)

        # Flatten vars and patches -> (batch_size, n_vars * n_patches, dim)
        enc_out_sh_pre = enc_out_sh_pre.reshape(enc_out_sh_pre.shape[0], enc_out_sh_pre.shape[1] * enc_out_sh_pre.shape[2], enc_out_sh_pre.shape[3])
        ##print("************************* enc_out_pre.shape after flattening: ", enc_out_pre.shape)
        ...
        enc_out_sh_pre = torch.reshape(
            enc_out_sh_pre, (-1, n_vars_flatten, int(enc_out_sh_pre.shape[-2]), enc_out_sh_pre.shape[-1]))
        # z: [bs x nvars x d_model x patch_num]
