...
import optuna
from models import PatchTST_sepcmix_simple_flatten #PatchTST_cmix_simple_flatten_change_wo_sub # Replace with your PatchTST model import
import numpy as np
import argparse
import random
...
def objective(trial):
    ...
    parser.add_argument('--root_path', type=str, default='./dataset/rag_data/data/', help='root path of the data file')
    parser.add_argument('--data_path', type=str, default='covid_till14May22.csv', help='data file')
    parser.add_argument('--features', type=str, default='MS',
                        help='forecasting task, options:[M, S, MS]; M:multivariate predict multivariate, S:univariate predict univariate, MS:multivariate predict univariate')
    parser.add_argument('--target', type=str, default='new_deaths', help='target feature in S or MS task')
    ...
    # forecasting task
    parser.add_argument('--seq_len', type=int, default=12, help='input sequence length') #96
    ...
    parser.add_argument('--d_model', type=int, default=2048, help='dimension of model') #2048
    ...
    parser.add_argument('--stride', type=int, default=1, help='stride')#8
    ...
    # TimeXer
    parser.add_argument('--patch_len', type=int, default=2, help='patch length') #16
    ...
    #args.seq_len = trial.suggest_int("seq_len", 2, 36)
    #args.patch_len = trial.suggest_int("patch_len", 1, args.seq_len)
    ...
    args.heads = trial.suggest_int("heads", 1, 4)
    args.dropout = trial.suggest_float("dropout", 0.1, 0.5)

    # Define model
    model = PatchTST_sepcmix_simple_flatten.Model(args).float()  #PatchTST_cmix_simple_flatten_change_wo_sub.Model(args).float()
    device = torch.device("cuda:0")
    ...
    test_data_set, test_loader = data_provider(args, "test")
    ...
    for epoch in range(args.train_epochs):  # Keep short for tuning
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_corr) in enumerate(train_loader):
            ...
            batch_corr = batch_corr.float().to(device)
    ...
    model.eval()
    with torch.no_grad():   
        preds = []; trues=[]; preds_inverse =[]; trues_inverse=[]       
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_corr) in enumerate(test_loader):
            ...
            shape = batch_y.shape
            preds_inverse.append(test_data_set.inverse_transform(outputs.reshape(outputs.shape[0] * outputs.shape[1], -1)).reshape(outputs.shape)[:, :, f_dim:])
            trues_inverse.append(test_data_set.inverse_transform(batch_y.reshape(shape[0] * shape[1], -1)).reshape(shape)[:, :, f_dim:])
        ...
        smape_return = np.mean(np.array(smape_list))    
    return smape_return #total_loss / len(train_loader)
