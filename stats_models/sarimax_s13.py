...
def sarima_forecast(train_series, horizon, model,exog):
    """
    Inputs:
    ...
    """
    forecast = model.get_forecast(steps=horizon,exog=exog)
...
def evaluate_sarima(time_series, order, seasonal_order, horizon, exog):
    """
    Inputs:
    ...
    for i in range(len(test_series) - horizon):
        ...
        # print(i)
        model = SARIMAX(time_series[horizon:split_idx + i], order=order, seasonal_order=seasonal_order,exog=exog_series[:split_idx + i-horizon,:])
        model_fitted = model.fit(disp=False)
        forecast = sarima_forecast(time_series[:split_idx + i], horizon, model_fitted,exog=exog_series[split_idx +i-horizon:split_idx +i + horizon-horizon,:])
        ...
        for q in range(1,5):
            for P in range(0,1):
                for Q in range(0,1):
                    ...
                    # Evaluate SARIMAX model for different horizons
                    for horizon in range(12, 13):
                        smape_value = evaluate_sarima(time_series, order, seasonal_order, horizon,exog=np.log1p(df[['NUM. OF PROVIDERS','OT']].values) )
